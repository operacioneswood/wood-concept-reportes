...
/**
 * PC weights per role per normalized status.
 * base: PC value | useLevel: multiply by level multiplier if true.
 */
const PC_WEIGHTS = {
  senior: {
    'diseno':                   { base: 4,   useLevel: false },
    'asignado':                 { base: 4,   useLevel: false },
    'en dibujo':                { base: 2,   useLevel: true  },
    'revision de constructivo': { base: 3,   useLevel: true  },
    'enviado a aprobacion':     { base: 0.5, useLevel: false },
    'proximos a entrar':        { base: 1,   useLevel: false },
  },
  junior: {
    'diseno':                   { base: 0,   useLevel: false },
    'asignado':                 { base: 0,   useLevel: false },
    'en dibujo':                { base: 3,   useLevel: true  },
    'revision de constructivo': { base: 4,   useLevel: true  },
    'enviado a aprobacion':     { base: 0.5, useLevel: false },
    'proximos a entrar':        { base: 1,   useLevel: false },
  },
};

/** Level multiplier breakpoints — first max ≥ nivel wins. */
const PC_LEVEL_MULT = [
  { max: 2,  mult: 0.7 },
  { max: 4,  mult: 1.0 },
  { max: 6,  mult: 1.3 },
...
/**
 * Load zone thresholds per role.
 * 0–green  → Disponible | green+1–yellow → Carga media | yellow+1+ → Saturado
 */
const PC_ZONES = {
  senior: { green: 28, yellow: 45 },
  junior: { green: 17, yellow: 30 },
};

/** Normalized statuses relevant for workload (everything else is ignored). */
const PC_STATUSES = new Set([
  'diseno', 'asignado', 'en dibujo',
  'enviado a aprobacion', 'revision de constructivo', 'proximos a entrar',
]);
